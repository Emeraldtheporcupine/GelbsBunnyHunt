...
controller.B.onEvent(ControllerButtonEvent.Pressed, function () {
    if (Title == true == (Rated == false)) {
        Fade(0, 1, 0)
        music.stopAllSounds()
        music.play(music.createSoundEffect(WaveShape.Sawtooth, 1138, 1137, 255, 0, 500, SoundExpressionEffect.None, InterpolationCurve.Linear), music.PlaybackMode.InBackground)
        Title = false
...
sprites.onOverlap(SpriteKind.Player, SpriteKind.Spike, function (sprite, otherSprite) {
    ...
    if (info.life() < 1) {
        ...
        sprite.setImage(assets.image`dead`)
        controller.moveSprite(sprite, 0, 0)
        timer.after(4000, function () {
            Direction = -1
            controller.moveSprite(Gelb, 75, 0)
            sprite.setImage(assets.image`Gelb L`)
            SetupLevel()
        })
    }
})
